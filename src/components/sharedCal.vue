<template>
  <div class="wrapper" v-if="$store.state.sharedDataReady">
    ...
    <cat-box :admin="user['admin']" :calID="calID"/>

    <div v-if="weekView && $store.getters.getInfo(calID) !== undefined && $store.getters.getSharedUsers !== undefined">

      <div class="grid" v-for="(month,index) in $store.getters.getInfo(calID)">
        ...
        <template v-for="(userCal, jindex) in month">
          <template v-if="jindex===0 && activeMonths.includes(index)" v-for="day in userCal">
            <div  class="datum" :class="{ 'currentDate': (day.day==currentDay && day.month-1 == currentMonth && year == currentYear)}"
                  v-if="activeMonths.includes(index)" @click="sortBlocks(day.day, day.month)"
    ...
    <div v-if="!weekView">
      <div class="gridMonth" v-for="(month,index) in $store.getters.getInfo(calID)">
        ...
        <template v-for="(userCal, jindex) in month" >
          <template v-if="jindex==0 && activeMonths.includes(index)" v-for="day in userCal">
            <div  class="datum" :class="{ 'currentDate': (day.day==currentDay && day.month-1 == currentMonth)}"
                  :style="{'grid-column': day.day +1, 'grid-row': jindex+1}" @click="sortBlocks(day.day, day.month)">
              {{ day.day }}
            </div>
    ...
    <day-box :unreg="0" shared="true"/>
    <cat-edit-box :unreg="0" :calID="calID" :shared="true"/>
    <infobox/>
...
<script>
  ...
  export default {
    ...
    created () {
      this.$store.commit('set_sharedDataReady', false)
    ...
    methods: {
      ...
      sortBlocks (day, month) {
        var order = []
        for (const [key, value] of Object.entries(this.$store.getters.getInfo(this.calID)[month-1])){
          order.push([key, value[day-1]['cat_id']])
        }
        order.sort(function(a, b) {
          return a[1] - b[1];
        });
        this.$store.commit('reorderShared', {order:order, calID: this.calID})
        this.$store.commit('reorderUsers', order)
      },
